...
describe("StableCoin", function () {
  let ethUsdPrice: any, feeRatePercentage: number;
  let StableCoin: StableCoin;

  this.beforeEach(async () => {
    feeRatePercentage = 3;

    const OracleFactory = await ethers.getContractFactory("PriceConsumerV3");
    const ethUsdOracle = await OracleFactory.deploy();
    ethUsdPrice = await ethUsdOracle.getLatestPrice();
  ...
  it("Should allow minting", async function () {
    ...
    console.log("expected", expectedMintAmount);
